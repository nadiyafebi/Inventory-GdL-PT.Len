...
export default function EksporBookingModal({ isOpen, onClose, onExport, total = 0 }) {
  ...
  const handleExport = () => {
    onExport?.({ format, tanggalMulai, tanggalSelesai });
    onClose?.();
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Ekspor Booking Ruangan</h2>
            <p className="text-xs text-gray-400">
              Unduh data pemesanan ruangan sesuai rentang tanggal yang dipilih
            </p>
          </div>
        ...
        <div className="flex flex-col gap-4 text-sm">
          ...
          <p className="text-xs text-gray-400">
            Total data yang akan diekspor: <span className="font-semibold text-gray-600">{total}</span> booking
          </p>
