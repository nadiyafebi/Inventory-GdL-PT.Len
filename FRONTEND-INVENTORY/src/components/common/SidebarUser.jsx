...
const SidebarUser = ({
  ...
  const [userData, setUserData] = useState({ nama: 'User', email: 'user123@gmail.com' });
    ...
    if (storedUser) {
      try {
        ...
        setUserData({
          ...
          email: parsed.email || 'user123@gmail.com'
        });
      } catch (e) {
        console.error('Gagal parsing userData', e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isMobileOpen && isMobile) {
        const sidebar = document.getElementById('mobile-sidebar-user');
        const toggleBtn = document.getElementById('sidebar-toggle-user');
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 overflow-hidden min-w-0">
            ...
            <div className="overflow-hidden min-w-0">
              <p className="text-xs font-bold text-gray-800 truncate">{userData.nama}</p>
              <p className="text-[10px] text-gray-500 font-medium truncate">{userData.email}</p>
            </div>
          </div>
          <button
            type="button"
